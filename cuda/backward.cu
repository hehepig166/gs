...
// backward steps contained in preprocess)
// compute dL_dmeans, dL_dcov
__global__ void computeCov2DCUDA_2(int P,
	const float3* means,
	const int* radii,
	const float* cov3Ds,
	const float h_x, float h_y,
	...
	const float* view_matrix,
	const float* dL_dconics,
	const float* dL_dconic33s,	// 2024-04-28 zzk
	float3* dL_dmeans,
	float* dL_dcov)
{
	auto idx = cg::this_grid().thread_rank();
	...
	glm::mat3 J = glm::mat3(h_x / t.z, 0.0f, -(h_x * t.x) / (t.z * t.z),
		0.0f, h_y / t.z, -(h_y * t.y) / (t.z * t.z),
		// 0, 0, 0);
		t.x / pl, t.y / pl, t.z / pl);	// 2024-04-28 zzk
...
renderCUDA_2(
	...
	const float2* __restrict__ points_xy_image,
	const float4* __restrict__ conic_opacity,
	const float* __restrict__ conic33s,				// 2024-04-24 zzk
	const float* __restrict__ colors,
	const float* __restrict__ final_Ts,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	...
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dconic33)				// 2024-04-24 zzk
{
	// We rasterize again. Compute necessary block info.
	auto block = cg::this_thread_block();
	const uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
